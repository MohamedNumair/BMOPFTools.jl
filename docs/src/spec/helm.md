...
BMOPFTools ships a deterministic, non-iterative 4-wire power-flow solver based
...
[`HelmResult`](@ref)). It complements the Ipopt-based `solve_pf`: no initial
guess, no convergence tuning, and — uniquely — a **certificate** when no
power-flow solution exists, plus an estimate of the distance to voltage
collapse from every solve.
...
zero-leakage block loses the coupling entirely. [`ybus_augmented`](@ref)
extends the [passive Ybus](nodal-admittance.md) with one **constraint row and
auxiliary current unknown per ideal coupling**:
...
Each row of ``\mathbf{A}`` is one [`IdealCoupling`](@ref) — a linear identity
...
[`ybus_linearized`](@ref), so the load model is byte-equivalent to the OPF's).
...
- **Loads: constant-power + constant-impedance parts only** (including those
  ...
  [`ybus_linearized`](@ref) fixed-point map or the OPF power flow for those;
